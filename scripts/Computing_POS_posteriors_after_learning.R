...
###database
load("~/data/DBs_all_compounds_only1entry_afterLearning.Rdata") ##database
###Priors
load("~/data/POSPriors_oneEntry_after_learning.Rdata") # Priors



#### adding unknowns in the connectivity matrices
Adducts.matrix <- DB2.structured.POS.updated$Add.M
...
Isotopes.matrix <- DB2.structured.POS.updated$Iso.M
...
Bio.matrix <- DB2.structured.POS.updated$Bio.M
...
Post.real.Pos.oneEntry.ADD.ISO.BIO.Int <- IPA.sampler.Add.Iso.Bio.int_noPot(P=Prior.red,Add = Adducts.matrix,
                                                                            Bio = Bio.matrix,
...
tmp.Post <- Prior.Pos
tmp.Post[ind.masses.kept, ind.formulas.kept] <- Post.real.Pos.oneEntry.ADD.ISO.BIO.Int$Post
Post.real.Pos.oneEntry.ADD.ISO.BIO.Int$Post <- tmp.Post
rm(tmp.Post)

tmp.allsampcomp <- matrix(0,2000, nrow(Prior.Pos))
tmp.allsampcomp[,ind.masses.kept] <- Post.real.Pos.oneEntry.ADD.ISO.BIO.Int$allsampcomp
Post.real.Pos.oneEntry.ADD.ISO.BIO.Int$allsampcomp <- tmp.allsampcomp
rm(tmp.allsampcomp)

save(Post.real.Pos.oneEntry.ADD.ISO.BIO.Int, 
     file="~/Desktop/new_IPA_2017/experiment_data_analysis/Post_real_oneEntry_ADD_ISO_BIO_Int_afterLearning2.Rdata")
