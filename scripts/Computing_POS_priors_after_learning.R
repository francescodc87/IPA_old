...
load("~/data/DBs_all_compounds_only1entry_afterLearning.Rdata")
source('~/R/IPA_massbased_priors.R')

compounds.mass = DB2.structured.POS.updated$exact.masses.table[,4]
### for the prior I only need the Masses
Prior.Pos <- IPA.massbased.prior(Masses[ind.kept],
                                 compounds.mass = compounds.mass,ppm = 3,
                                 unknown.prob=0.05, compounds.id=1:length(compounds.mass),v=T)
rownames(Prior.Pos) <- ids[ind.kept]
...
#save(Prior.Pos,Prior.List, file="/data/POSPriors_oneEntry_after_learning.Rdata")
